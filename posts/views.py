...
from posts.forms import CommentForm, GroupForm, PostForm
...
def index(request: WSGIRequest) -> HttpResponse:
    ...
    context = {
        'page_obj': build_page_from_posts(
            request,
            Post.objects.select_related('author').exclude(
                author__username='user_deleted'),
        ),
    }
...
def group_list(request: WSGIRequest, slug: str) -> HttpResponse:
    ...
    context = {
        ...
        'page_obj': build_page_from_posts(
            request,
            group.posts.exclude(author__username='user_deleted'),
        ),
    }
...
def profile(request: WSGIRequest, username: str):
    ...
    context = {
        'author': user,
        'page_obj': build_page_from_posts(request, user.posts.exclude(
            author__username='user_deleted'), ),
